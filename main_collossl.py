...
from train.train_encoder_collossl import EncoderTrainerColloSSL, load_pretrained_collossl_encoder
...
if __name__ == '__main__': 
    ...
    # Enable ColloSSL mode
    args.collossl_mode = True
    
    # Device channel mapping is now automatically loaded in config.py

    # Timestamp
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    args.timestamp = timestamp
    
    logger = Logger(f"collossl_{args.encoder_type}_{args.classifier_type}")

    # Random Seed
    set_seed(args.seed)
    logger.info(f"Random Seed: {args.seed}")
    logger.info(f"Using ColloSSL with encoder type: {args.encoder_type}, classifier type: {args.classifier_type}")
    logger.info(f"Anchor device: {args.anchor_device}, Available devices: {args.available_devices}")
    ...
    logger.info(f"Starting {len(fold_range)}-fold ColloSSL training")
    ...
    for fold_idx in fold_range:
        ...
        encoder_save_path = os.path.join(args.encoder_save_path, f"collossl_{args.encoder_type}_{timestamp}", fold_dir)
        classifier_save_path = os.path.join(args.classifier_save_path, f"collossl_{args.encoder_type}_{args.classifier_type}_{timestamp}", fold_dir)
        ...
        with open(os.path.join(classifier_save_path, "fold_info.txt"), "w") as f:
            f.write(f"Training Mode: ColloSSL\n")
            ...
            f.write(f"Anchor Device: {args.anchor_device}\n")
            f.write(f"Available Devices: {args.devices}\n")
            f.write(f"Device Channel Mapping: {args.device_channel_mapping}\n")
            f.write(f"ColloSSL Parameters:\n")
            f.write(f"  - Temperature: {args.collossl_temperature}\n")
            f.write(f"  - MMD Kernel: {args.mmd_kernel}\n")
            f.write(f"  - MMD Sigma: {args.mmd_sigma}\n")
            f.write(f"  - Batch Size: {args.batch_size}\n")
            f.write(f"  - Negative Sample Size: {args.neg_sample_size}\n")
            f.write(f"  - Device Selection Metric: {args.device_selection_metric}\n")
            f.write(f"  - Device Selection Strategy: {args.device_selection_strategy}\n")
            f.write(f"  - Device Channel Mapping: {args.device_channel_mapping}\n")
            f.write(f"Timestamp: {timestamp}\n")
            f.write(f"Run ID: {run_id}\n")

        # ============Step 1: ColloSSL Encoder Training============
        if args.train_encoder:
            logger.info(f"Training ColloSSL encoder for fold {fold_idx+1}, test subject {current_test_subject}")
            
            # Create ColloSSL encoder
            encoder = create_collossl_encoder(args)
            encoder_trainer = EncoderTrainerColloSSL(args, encoder, encoder_save_path)
            encoder = encoder_trainer.train(train_loader, valid_loader)
            
            logger.info(f"ColloSSL encoder training completed for fold {fold_idx+1}")
        ...
        if args.train_classifier:
            ...
            # Create ColloSSL encoder for classifier training
            encoder = create_collossl_encoder(args)
            ...
            logger.info(f"Loading ColloSSL encoder from: {encoder_checkpoint_path}")
            encoder = load_pretrained_collossl_encoder(encoder, encoder_checkpoint_path)
            
            # Create and train classifier - use base encoder for compatibility
            # Extract base encoder from ColloSSL wrapper for classifier
        ...
        if args.test:
            ...
            if not args.train_classifier:
                ...
                # Create classifier with base encoder
                base_encoder = encoder.base_encoder
                classifier = create_classifier(args, base_encoder)
    ...
    if args.test and len(results['subject_id']) > 0:
        # Mark as ColloSSL
        args.encoder_type = f"collossl_{args.encoder_type}"
        save_results_summary(results, args, timestamp)
        
        # Log summary statistics
        logger.info("="*50)
        logger.info("ColloSSL Results Summary")
        logger.info("="*50)
        logger.info(f"Mean Accuracy: {np.mean(results['accuracy']):.4f} ± {np.std(results['accuracy']):.4f}")
        logger.info(f"Mean F1-Weighted: {np.mean(results['f1_weighted']):.4f} ± {np.std(results['f1_weighted']):.4f}")
        logger.info(f"Mean F1-Macro: {np.mean(results['f1_macro']):.4f} ± {np.std(results['f1_macro']):.4f}")
        logger.info(f"Mean F1-Micro: {np.mean(results['f1_micro']):.4f} ± {np.std(results['f1_micro']):.4f}")
        logger.info("="*50)
    
    logger.info("All ColloSSL processes completed successfully.") 