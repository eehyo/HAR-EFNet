...
from train.train_encoder_masked import create_masked_encoder, load_pretrained_masked_encoder, MaskedReconstructionTrainer
...
if __name__ == '__main__': 
    ...
    # Force masked reconstruction mode
    args.masked_mode = True
    
    # Set mask choice probability
    args.mask_choice = MASK_CHOICE
    
    # Timestamp
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    args.timestamp = timestamp
    
    logger = Logger(f"masked_har_{args.encoder_type}_{args.classifier_type}")

    # Random Seed
    set_seed(args.seed)
    logger.info(f"Random Seed: {args.seed}")
    logger.info(f"Using Masked Reconstruction SSL pretraining")
    logger.info(f"Encoder type: {args.encoder_type}, Classifier type: {args.classifier_type}")
    logger.info(f"Mask probability: {args.mask_choice}")
    ...
    for fold_idx in fold_range:
        ...
        encoder_save_path = os.path.join(args.encoder_save_path, f"masked_{args.encoder_type}_{timestamp}", fold_dir)
        classifier_save_path = os.path.join(args.classifier_save_path, f"masked_{args.encoder_type}_{args.classifier_type}_{timestamp}", fold_dir)
        ...
        with open(os.path.join(classifier_save_path, "fold_info.txt"), "w") as f:
            ...
            f.write(f"Pretext Task: Masked Reconstruction\n")
            f.write(f"Encoder Type: {args.encoder_type}\n")
            f.write(f"Classifier Type: {args.classifier_type}\n")
            f.write(f"Mask Probability: {args.mask_choice}\n")
            f.write(f"Timestamp: {timestamp}\n")
            f.write(f"Run ID: {run_id}\n")

         # ============Step 1: Masked Reconstruction Encoder Training============
        if args.train_encoder:
            logger.info(f"Training masked reconstruction encoder for fold {fold_idx+1}, test subject {current_test_subject}")
            
            encoder = create_masked_encoder(args)
            encoder_trainer = MaskedReconstructionTrainer(args, encoder, encoder_save_path)
            encoder = encoder_trainer.train(train_loader, valid_loader)
            
            logger.info(f"Masked reconstruction encoder training completed for fold {fold_idx+1}")
        ...
        if args.train_classifier:
            ...
            logger.info(f"Loading masked encoder from: {encoder_checkpoint_path}")
            encoder = load_pretrained_masked_encoder(encoder, encoder_checkpoint_path)
